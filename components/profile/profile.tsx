...
interface ProfileProps {
  savedName: { firstName: string; lastName: string };
  setSavedName: (name: { firstName: string; lastName: string }) => void;
}

export default function Profile({savedName, setSavedName}: ProfileProps){
  ...
  const handleSave = () => {
    ...
    setSavedName(updatedName);  // Trigger parent update
  };

  // useEffect(() => {
  //   console.log("Updated savedName:", savedName);
  // }, [savedName]);
  ...
  //   // localStorage.setItem("savedName", JSON.stringify(updatedName));
  //   // console.log("Profile: Saving name:", updatedName);
  //   try {
  //     setSavedName(updatedName);
  //     console.log("Profile: Saving name:", updatedName);
          ...
          <div className="w-[343px] md:w-[721px] lg:w-[808px] h-[739px] border-b border-b-[#D9D9D9] p-10 rounded-xl bg-white">
            ...
            <div className="bg-[#FAFAFA] p-5 flex flex-col gap-3">
              <div className="flex w-full gap-4 items-center">
                ...
                <input type="text" placeholder="e.g. john" value={firstName} onChange={(e)=> setFirstName(e.target.value)} className="bg-white w-full rounded-lg p-2 outline-none border-none border border-[#D9D9D9] text-[#333] text-base"/>
              </div>
              <div className="flex w-full gap-4 items-center">
                <label className="w-1/2 text-[#737373] text-base">Last name*</label>
                <input type="text" placeholder="e.g. Appleseed" value={lastName} onChange={(e)=> setLastName(e.target.value)} className="bg-white w-full rounded-lg p-2 outline-none border-none border border-[#D9D9D9] text-[#333] text-base"/>
              </div>
              <div className="flex w-full gap-4 items-center">
                <label className="w-1/2 text-[#737373] text-base">Email</label>
                <input type="email" placeholder="e.g. email@example.com" value={email} onChange={(e)=> setEmail(e.target.value)} className="bg-white w-full rounded-lg p-2 outline-none border-none border border-[#D9D9D9] text-[#333] text-base"/>
              </div>
            </div>
          </div>
          <div className=" w-[343px] md:w-[721px] lg:w-[808px] h-[94px] flex justify-end items-center px-10 py-6 rounded-xl bg-white">
            <button onClick={handleSave} className="px-7 py-3 text-base font-semibold text-white bg-[#633CFF] rounded-lg">
              Save
            </button>
          </div>
        </div>
